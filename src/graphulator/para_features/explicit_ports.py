...
class LineInputDialog(QDialog):
    ...
    def __init__(self, line=None, default_label='TL1', parent=None):
        ...
        self.alpha_spin = spin(line.get('alpha_uniform', 0.0), 0.0, 100.0, 6,
                               0.001,
                               "One-way amplitude attenuation alpha*l "
                               "[nepers]; maps to per-mode "
                               "B_int = (2/pi) alpha FSR (verified against "
                               "lossy ABCD in test_uniform_loss)")
        form.addRow("\N{GREEK SMALL LETTER ALPHA} [Np]:", self.alpha_spin)
...
class ExplicitPortsMixin:
    ...
    def add_port(self, label=None, pos=(0.0, 0.0), monitored=True):
    ...
    def _maybe_handle_attachment_click(self, event):
        ...
        Returns True when the click was consumed (port clicked, or a node
        clicked while a port is pending attachment).
        """
        ...
        if port is not None:
            if self._attach_pending_port is port:
                ...
                print(f"Attachment cancelled for '{port['label']}'")
            else:
                self._attach_pending_port = port
                # a plain edge start makes no sense from a port
        ...
        if self._attach_pending_port is not None:
            ...
            if node is not None:
                ...
                print(f"✓ Attached '{port['label']}' \N{RIGHTWARDS ARROW} "
                      f"'{node['label']}' (edit rate in the Ports panel)")
                self._attach_pending_port = None
                if hasattr(self, 'properties_panel'):
    ...
    def _port_geometry(self, port):
        """(x, y, w, h, apex_x, lead_tip_x) in data units."""
        r = self.node_radius
        x, y = port['pos']
        w = PORT_BODY_W * r
        h = PORT_BODY_H * r
    ...
    def _find_port_at_position(self, x, y):
        ...
        for port in reversed(self.ports):
            ...
            if (px - w / 2 <= x <= apex_x) and (py - h / 2 <= y <= py + h / 2):
                return port
        return None
    ...
    def _find_line_at_position(self, x, y):
        ...
        for line in reversed(self.line_resonators):
            ...
            h = LINE_BODY_H * r
            if (lx - w <= x <= lx + w) and (ly - h <= y <= ly + h):
                return line
        return None
    ...
    def _find_attachment_at_position(self, x, y, tol=None):
        ...
        for port in self.ports:
            _, py, _, _, _, lead_tip_x = self._port_geometry(port)
            p0 = np.array([lead_tip_x, py])
            for att in port['attachments']:
    ...
    def _draw_ports_and_lines(self, ax=None):
        ...
        for port in self.ports:
            ...
            ax.add_line(mlines.Line2D(
                [apex_x, lead_tip_x], [y, y], color=edge_color,
        ...
        for line in self.line_resonators:
            ...
            ax.text(lx, ly + h + 0.35 * r, line['label'], ha='center',
                    va='bottom', fontsize=10, fontweight='bold',
                    color=edge_color, zorder=12)
            ax.text(lx, ly - h - 0.35 * r, sub, ha='center', va='top',
